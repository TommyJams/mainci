        ...
        <section id="left" >
            <div id="pageContainer" style = "width:100%; height:100%;">
                ...
                ?>
                    <table id="framemenu" >
                            <tr>
                            <td width="25%"><center><a href="javascript:;" onClick="showEditFrame('frameprofessional');"><h1>Professional</h1></a></td>
                            <td width="25%"><center><a href="javascript:;" onClick="showEditFrame('framesocial');"><h1>Social</h1></a></center></td>
                            <td width="25%"><center><a href="javascript:;" onClick="showEditFrame('framecontact');"><h1>Contact</h1></a></center></td>
                            <td width="25%"><center><a href="javascript:;" onClick="showEditFrame('frameabout');"><h1>About</h1></a></center></td>
                            <tr>
                    </table>
                    <div id="frameprofessional">
                        <form action="" method="POST" class="cleanForm" id="professionalForm" >
                            <fieldset>
                                <p>
                                    <? if($type == 'Artist') { print("<label for='designation'>Your Role:</label>");}
                                       elseif($type == 'Promoter')    { print("<label for='designation'>Designation:</label>");}
                                    ?>
                                    
                                    <input type="text" id="full-name" name="designation" value="<? print($designation); ?>"  pattern="^[a-zA-Z0-9/ ,-_.:;?]{3,50}$" autofocus />
                                    <br>
                                    <? if($type == 'Artist') { print("<em>e.g. Guitar, Vocals</em>");}
                                       elseif($type == 'Promoter')    { print("<em>e.g. Manager</em>");}
                                    ?>
                                </p>
                                <p>
                                    <? if($type == 'Artist') { print("<label for='organization'>Band Name: <span class='requiredField'>*</span></label>");}
                                       elseif($type == 'Promoter')    { print("<label for='organization'>Organization: <span class='requiredField'>*</span></label>");}
                                    ?>
                                ...
                                <p>
                                    <? if($type == 'Artist') { print("<label for='genre'>Genre: </label>");}
                                       elseif($type == 'Promoter')    { print("<label for='genre'>Style: </label>");}
                                    ?>
                                    <input type="text" id="genrename" name="genre" pattern="^[a-zA-Z0-9/ ,-_.:;?]{3,50}$" value="<? print ($genre); ?>"/>
                                    <br>
                                    <? if($type == 'Artist') { print("<em>e.g. Acoustic, Progressive Rock</em>");}
                                       elseif($type == 'Promoter')    { print("<em>e.g. Cafe, Lounge or Rock, Electronic</em>");}
                                    ?>
                                </p>
                                <center>
